...
- Batch related research/edits under the root-level `Cody Temp/.tmp-trusted-*` staging pattern (manifest, receipt, document-outline, document-result, document-text) before writing to canonical repo files. The `Cody Temp/` folder is local-only and ignored by Git; canonical evidence must be promoted to tracked files before it is relied on as shared project state. Root-level `.tmp-trusted-*` batches created before 2026-09-14 were moved intact into `Cody Temp/`; historical references to those batch names resolve there. This is already Cody's default behavior for read/research tasks — this section makes it explicit and required, not incidental.
