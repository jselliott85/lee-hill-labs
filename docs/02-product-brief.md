...
- Every approved adult member may create and claim structured Requests and Offers. A Request claim means `I'll help`; an Offer claim means `I'll take you up on that`. Newly created Requests and Offers are visible in-app and permitted to generate device notifications for approved members of the relevant trusted community in MVE-1.
...
- Device notifications are permitted for new community Requests and Offers, approved direct-participant activity, and explicitly approved incident event/source combinations. MVE-1 uses OS-only notification control with On recommended; no in-app master switch or category preferences ship. OS permission must actually be granted, delivery is never guaranteed, and unknown incident combinations fail closed to no push. Granular in-app settings are deferred until after MVE-1.
...
## MVE-1 No-Location Event Limitation

When no usable resident-accepted home location exists, both Event/alert notifications and personalized sourced Events in the feed are unavailable. Clearly explain both consequences before continuing setup and provide an address-correction path rather than implying there are no Events. Community Requests, Offers and messages remain available under existing membership/access rules. A usable approximate home point does not require a separate postal-validation pass. Approved supersession: 2026-09-09 decision log.
