...
- MVE-1 is the first validation slice within the MVP. Its approved first journey is household-first: sourced incident context associated with the saved address, a practical request, trusted-member claim, private request thread, deliberate time-limited information sharing, requester-only close, and recovery coordination.
