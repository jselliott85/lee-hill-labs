...
- **Home Status:** Home Status remains on Home even without a usable mapped coordinate and is updated directly inside the Home Signals section. It shows the resident's latest voluntary status and timestamp with inline `Home`/`Away` controls and no freeform field. Provisionally, `Home` means someone in the reporting resident's household is home; `Away` means the reporting resident says the household is away. Adults at the same address report independently. The two-state taxonomy is an approved starting point, not a final state model.
