...
## 2026-09-10 — A1 Round 2 Notification Action, Critical Location, and Placeholder Copy

- **Permission interaction:** S02/S02b presents one LHL action that opens the native OS notification decision; there is no separate app-level `Not now` or defer action (`devlog`). Native Allow and Deny both continue setup. On Android, dismissing the native prompt without choosing leaves permission not determined and pushes unavailable, but also continues setup; later recovery uses OS settings. None of these outcomes blocks app access.
- **Critical location rule:** The complete saved-home address remains mandatory resident input. A usable matched or resident-accepted home location is critical to the full approved incident-matrix Event/alert experience, covering all approved alerts and Event Types but excluding community actions (`devlog`). Without a usable location, Requests, Offers, claimed-item messages, and other approved community functions remain available under their normal access rules. Meaningful address match/find/validation failure rates are a major product-functionality and adoption risk to measure; provider, threshold, and metric selection remain open.
- **Privacy rule:** Address and access instructions remain private from ordinary community visibility. Address may be used internally for Event relevance and is included in a deliberate request-specific share; access instructions are shared only when selected under the existing temporary-sharing flow.
- **Copy baseline:** Approved current placeholder: `LHL complements official alerts by presenting qualifying information from approved sources; it never replaces official alerts.` Final wording remains Gate 3 work.
- **Low-fi/review boundary:** Remove the currently drawn S02/S02b app-level defer actions only after the canonical record and remaining bounded Claude re-review. S03d already returns to S03a. Other comments remain review input, not final layout/copy approval. No A1 completion, A2, global Pass 2, provider, implementation, task completion, or gate closure is approved.
- **Source:** John's three explicit answers after completing A1 Round 2 Figma review, 2026-09-10; controlling entry in `docs/04-decision-log.md`.
