...
On September 11, John confirmed the current outreach sequence: hold the next Boulder ODM message until the low-fidelity screens are usable as concrete artifacts for a bounded proposal. This artifact-first sequence is intentional, not inactivity or evidence that access is less important. The screens should make the requested resident-facing use case easier for Steve and the county policy group to evaluate; they do not imply source approval, authorize access, or replace the requirement that John review the exact proposal before it is sent.

The same proposal must ask Steve about lifecycle identity, not access alone. Protective Action has no VTEC-like identity rule available to LHL, and the existing candidate-source records correctly leave lifecycle semantics unresolved. Ask which stable identifier follows one protective action across updates; whether an Evacuation Warning and Evacuation Order coexist or one upgrades/terminates the other; whether Shelter-in-Place is independent; whether Repopulation / Lifting terminates a prior action or is a separate record; which fields/actions express correction, cancellation, expiry, and closure; and whether custom geometry changes preserve identity. LHL will not infer these relationships from labels, timing, severity, or overlapping polygons. This is a question set for the founder-reviewed outreach, not permission to inspect or implement the unapproved interface.
